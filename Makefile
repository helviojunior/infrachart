...
PACKAGENAME="github.com/helviojunior/infrachart"
...
windows: get-version	## Make Windows x86 and x64 Binaries
	@for ARCH in ${ARCHS}; do \
		...
		GOOS=windows GOARCH=$${ARCH} go build -a -ldflags "$$(cat ${TARGET}/.ldflags)" -o ${TARGET}/infrachart_windows_$${ARCH}.exe || exit 1 ;\
	done; \
	echo "Done."
...
linux: get-version	## Make Linux x86 and x64 Binaries
	@for ARCH in ${ARCHS}; do \
		...
		GOOS=linux GOARCH=$${ARCH} go build -a -ldflags "$$(cat ${TARGET}/.ldflags)" -o ${TARGET}/infrachart_linux_$${ARCH} || exit 1 ;\
	done; \
	echo "Done."
...
mac: get-version	## Make Darwin (Mac) x86 and x64 Binaries
	@for ARCH in ${ARCHS}; do \
		if [ "$${ARCH}" != "386" ]; then \
			...
			GOOS=darwin GOARCH=$${ARCH} go build -a -ldflags "$$(cat ${TARGET}/.ldflags)" -o ${TARGET}/infrachart_darwin_$${ARCH} || exit 1 ;\
		fi;\
	done; \
	echo "Done."
...
local: get-version	## Compila para a plataforma e arquitetura local detectada
	...
	GOOS=$$OS GOARCH=$$ARCH go build -ldflags "$$(cat ${TARGET}/.ldflags)" -o ${TARGET}/infrachart_$$OS\_$$ARCH || exit 1 ; \
	echo "Done."
